...
import (
	...
	docs "github.com/lihongjie0209/scheduler-service/docs"
	...
	"github.com/lihongjie0209/scheduler-service/internal/ratelimit"
...
func NewServer(lc fx.Lifecycle, cfg config.Config, handler *Handler, authService *auth.Service, authorizer platformauthz.Authorizer, limiter *ratelimit.Limiter, metrics *observability.Metrics, tracing *observability.Tracing, logger *slog.Logger) (*http.Server, error) {
	...
	api.POST("/scheduler/jobs/create", handler.CreateJob)
	api.POST("/scheduler/jobs/update", handler.UpdateJob)
	api.POST("/scheduler/jobs/delete", handler.DeleteJob)
	api.POST("/scheduler/jobs/get", handler.GetJob)
	api.POST("/scheduler/jobs/list", handler.ListJobs)
	api.POST("/scheduler/jobs/trigger", handler.TriggerJob)
	api.POST("/scheduler/executions/get", handler.GetExecution)
	api.POST("/scheduler/executions/list", handler.ListExecutions)
